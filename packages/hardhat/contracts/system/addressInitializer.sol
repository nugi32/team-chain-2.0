...
contract addressInitializer is systemAddressUtils{
    ...
    // initialization flags to ensure each step runs only once
    bool public addressesSet;
    bool public initializedAll;
    ...
    function __addressDataInitializer(
    ...
    ) external callerZeroAddr onlyOwner onlyOnce {
        ...
        if (addressesSet) revert addressInitializerError("Addresses already set");
        addressesSet = true;
    }

    function initializeAll() external onlyOwner onlyOnce callerZeroAddr {

        if (!addressesSet) revert addressInitializerError("Addresses not set");
        if (initializedAll) revert addressInitializerError("Already initialized");
