 ...
 */
contract stateVariable is MainAccesControlPipes, Pausable, systemAddressUtils {
//
    // =============================================================
    // Struct Definitions
    // =============================================================
...
//
    ...
    function __stateVariableAddressInit(        
        ...
        walletContract = IWallet(_systemWalletAddress);
        ...
        addressInitializerContract = address(0);

        emit StateVariableInitialized(
...
//
    ...
    event StateVariableInitialized(
...
//
    ...
    constructor(
        ...
        address _addressInitializerContract
    ) {
        ...
        if (_maxRewardInEther > _maxStakeInEther) revert InvalidMaxStakeAmount();
        ...
        if (_addressInitializerContract == address(0)) {
            revert ZeroAddressNotAllowed();
        } 
        ...
        addressInitializerContract = _addressInitializerContract;
    }
...
//
    ...
    function setComponentWeightPercentages(
        ...
        uint64 _deadlineScore,
        uint64 _revisionScore
    ) external onlyEmployes(addressUtils.accessControlAddress) whenNotPaused {
    ...
    // ---------------------------------------------------------
    // 1. Zero-value sanity check (optional but strongly advised)
    // ---------------------------------------------------------
    if (
        _low == 0 ||
        _middleLow == 0 ||
        ...
        revert ZeroCategoryNotAllowed();
    }

    // ---------------------------------------------------------
    // 2. Strict ascending order (ETH units, pre-normalization)
    // ---------------------------------------------------------
    if (
        _low >= _middleLow ||
        _middleLow >= _middle ||
    ...
    // ---------------------------------------------------------
    // 4. Enforce system-wide economic ceilings
    // ---------------------------------------------------------
    ...
    function setAddressUtils(
        address _accessControlAddress,
        address _systemWalletAddress
    ) external onlyOwner(addressUtils.accessControlAddress) whenNotPaused {
        if (_accessControlAddress == address(0) || 
            _systemWalletAddress == address(0)) {
            revert ZeroAddressNotAllowed();
        }
     ...
     */
    function changeAccessControl(address _newAccessControl) external onlyOwner(addressUtils.accessControlAddress) whenNotPaused {
     ...
     */
    function pause() external onlyOwner(addressUtils.accessControlAddress) {
        _pause();
     ...
     */
    function unpause() external onlyOwner(addressUtils.accessControlAddress) {
        _unpause();
