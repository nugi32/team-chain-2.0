...
import { AlertCircle, ArrowLeftRight, CheckCircle2, Loader2, Wallet } from "lucide-react";
...
export default function WalletConnectBlock() {
  ...
  const isConnectedToSepolia = isConnected && chainId === sepolia.id;

  useEffect(() => {
    if (connectedAddress && chainId !== sepolia.id && switchChain) {
  ...
  return (
    <ConnectButton.Custom>
      {({ account, chain, openConnectModal, mounted }) => {
        ...
        const iconBg = wrongNetwork ? "bg-amber-500/20" : allGood ? "bg-emerald-500/20" : "bg-indigo-500/20";
        ...
        const title = wrongNetwork ? "Wrong Network" : allGood ? "Wallet Connected" : "Connect Wallet — Required";
                      ...
                      <p className="text-xs text-gray-400 mt-0.5 leading-relaxed">
                        {wrongNetwork ? (
                          <span className="text-amber-400">
                            This app requires <span className="font-semibold">Sepolia</span>. Switch networks to
                            continue.
                          </span>
                        ) : allGood ? (
                          <span className="font-mono text-emerald-400 break-all">{account.address}</span>
                        ) : (
