    ...
    <div className="w-64 flex-shrink-0 hidden xl:flex flex-col gap-4">
            ...
            <div key={r.label} className="flex items-center gap-2">
              <div
                className={[
                  ...
                  r.done ? "bg-emerald-500/20 text-emerald-400" : "bg-gray-800 text-gray-700",
                ].join(" ")}
              >
      ...
      <div className="rounded-2xl border border-gray-800 bg-gray-900/60 overflow-hidden">
        ...
        <div className="p-4">
          ...
          <p className="text-[9px] text-gray-500 mb-2 leading-snug line-clamp-2">
            {data.objective || "One-line objective…"}
          </p>
          <div className="flex flex-wrap gap-1 mb-2">
            {data.badges.slice(0, 2).map(b => (
              <span
                key={b}
                className={["text-[8px] font-medium px-1.5 py-0.5 rounded border", BADGE_MAP[b] || ""].join(" ")}
              >
                {b}
              </span>
            ))}
          ...
          <div className="grid grid-cols-2 gap-1 mb-2">
            <div className="rounded border border-gray-800 bg-gray-900 p-1.5 text-center">
              <p className="text-[9px] font-semibold text-amber-300">
                {data.stakeRequired ? `Ξ ${data.stakeRequired}` : "—"}
              </p>
          ...
          <div className="flex flex-wrap gap-1">
            {data.skills.slice(0, 3).map(s => (
              <span key={s} className="text-[8px] px-1.5 py-0.5 rounded bg-gray-800 text-gray-500">
                {s}
              </span>
            ))}
          </div>
