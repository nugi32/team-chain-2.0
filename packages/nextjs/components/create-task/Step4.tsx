...
export default function Step4({
  data, onPublish, publishing, published,
}: {
  data: FormData;
  onPublish: () => void;
  publishing: boolean;
          ...
          <div className="p-5">
            ...
            <div className="flex flex-wrap gap-1 mb-3">
              {data.skills.slice(0, 5).map((s) => (
                <span key={s} className="text-[10px] px-2 py-0.5 rounded bg-gray-800 text-gray-400 border border-gray-700">{s}</span>
              ))}
              {data.skills.length > 5 && <span className="text-[10px] text-gray-600">+{data.skills.length - 5} more</span>}
            </div>
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-gray-600">{data.slots} slot{parseInt(data.slots) > 1 ? "s" : ""} • {data.category || "Uncategorized"}</span>
