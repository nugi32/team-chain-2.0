...
export default function Input({ value, onChange, placeholder, prefix, maxLength, type = "text" }: InputProps) {
  return (
    <div className="flex items-center rounded-xl border border-gray-800 bg-gray-900 focus-within:border-indigo-500/50 transition-colors overflow-hidden">
      {prefix && <span className="pl-3 text-xs text-gray-600 select-none flex-shrink-0">{prefix}</span>}
      <input
        type={type}
        value={value}
