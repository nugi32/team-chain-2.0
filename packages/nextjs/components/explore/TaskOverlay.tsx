...
const TIER_STYLES = {
  Bronze: {
    ...
    badge: "bg-amber-900/40  border-amber-700/50  text-amber-400",
    dot: "bg-amber-400",
    stat: "text-amber-400",
  },
  Silver: {
    card: "border-slate-500/30  bg-slate-800/20",
    badge: "bg-slate-700/40  border-slate-500/50  text-slate-300",
    dot: "bg-slate-400",
    stat: "text-slate-300",
  },
  Gold: {
    card: "border-yellow-600/30 bg-yellow-950/20",
    badge: "bg-yellow-900/40 border-yellow-600/50 text-yellow-400",
    dot: "bg-yellow-400",
    stat: "text-yellow-400",
  },
  Platinum: {
    card: "border-cyan-600/30   bg-cyan-950/20",
    badge: "bg-cyan-900/40   border-cyan-600/50   text-cyan-400",
    dot: "bg-cyan-400",
    stat: "text-cyan-400",
  },
} as const;
...
function CreatorCard({
    ...
    return (
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-mono text-gray-400">{shortAddr(creatorAddress) || "Unknown"}</p>
  ...
  const successColor = successRate >= 80 ? "text-emerald-400" : successRate >= 50 ? "text-yellow-400" : "text-gray-300";

  return (
    <div className={`rounded-xl border p-4 space-y-3.5 ${tierStyle ? tierStyle.card : "bg-gray-900 border-gray-800"}`}>
      {/* Avatar + name + tier */}
      <div className="flex items-center gap-3">
        {creator.profilePicture ? (
          ...
          <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-indigo-600 to-violet-600 flex items-center justify-center flex-shrink-0">
            <span className="text-sm font-bold text-white">{creator.name?.slice(0, 2).toUpperCase() ?? "??"}</span>
          </div>
        )}

        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <p className="text-sm font-semibold text-gray-100 truncate">{creator.name ?? shortAddr(creatorAddress)}</p>
            {tier && tierStyle && (
              <span
                className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded border text-[9px] font-bold uppercase tracking-wide ${tierStyle.badge}`}
              >
          ...
          <div className="flex items-center gap-2 mt-0.5">
            {creator.role && <span className="text-[10px] text-indigo-400 font-medium">{creator.role}</span>}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            ...
            {creator.skills!.length > 6 && (
              <span className="text-[10px] text-gray-600 self-center">+{creator.skills!.length - 6} more</span>
            )}
          </div>
        </div>
      )}
...
export default function TaskOverlay({ task, onClose }: { task: CompleteTaskOutput | null; onClose: () => void }) {
  ...
  const creatorAddress = task?.creator && task.creator !== ZERO_ADDRESS ? task.creator : (task?.owner ?? undefined);

  const { user: creator, loadingUser: loadingCreator } = useDashboardUserData(creatorAddress);
  ...
  // Parse description
  const desc = useMemo(() => (task?.description ? parseDescription(task.description) : null), [task?.description]);
  ...
  const displayName =
    task?.projectName || (task as any)?.title || (task?.smartContractId ? `Task #${task.smartContractId}` : "Task");
          ...
          >
            ...
            <div className="flex items-start justify-between gap-4 px-6 py-5 border-b border-gray-800">
              <div className="flex items-start gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h2 className="text-sm font-bold text-gray-100 leading-tight">{displayName}</h2>
                  <p
                    className={`text-xs mt-0.5 truncate ${
                      creator?.name ? "text-indigo-300 font-medium" : "text-indigo-400 font-mono"
                    }`}
                  >
                    {headerSubtitle}
                  </p>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              <div className="flex flex-wrap gap-2 items-center">
                <span
                  className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md border text-[10px] font-medium ${statusStyles(task.status)}`}
                >
              ...
              {/* Creator profile */}
              <div>
                <p className="text-[11px] uppercase tracking-wider text-gray-600 font-semibold mb-2">Creator</p>
                <CreatorCard creator={creator} creatorAddress={creatorAddress} loading={loadingCreator} />
              </div>

              {/* Objective */}
              {task.objective && (
                <div>
                  <p className="text-[11px] uppercase tracking-wider text-gray-600 font-semibold mb-2">Objective</p>
              ...
              {desc && (desc.summary || desc.header) && (
                <div>
                  <p className="text-[11px] uppercase tracking-wider text-gray-600 font-semibold mb-2">Description</p>
                  {desc.header && <p className="text-xs font-semibold text-gray-300 mb-1.5">{desc.header}</p>}
                  {desc.summary && <p className="text-sm text-gray-400 leading-relaxed">{desc.summary}</p>}
                  ...
                  {desc.footer && <p className="mt-2.5 text-xs text-gray-600 italic">{desc.footer}</p>}
                </div>
              )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    },
                    {
                      ...
                      label: "Est. Hours",
                      value: task.deadlineHours ? `${Number(task.deadlineHours)}h` : "N/A",
                      color: "text-gray-200",
                    },
                    {
                      ...
                      color: deadlineDays < 9999 && deadlineDays <= 3 ? "text-red-400" : "text-gray-200",
                    },
                  ].map(item => (
              ...
              <div>
                ...
                <div className="bg-gray-900 rounded-xl border border-gray-800 divide-y divide-gray-800">
                  <div className="flex items-center justify-between px-4 py-2.5">
                    ...
                    <span className="text-xs font-semibold text-indigo-400">{ethDisplay(task.memberStake)}</span>
                  </div>
                  <div className="flex items-center justify-between px-4 py-2.5">
                    <span className="text-xs text-gray-500">Min. Reputation</span>
                    <span className="text-xs font-semibold text-indigo-400">{task.minReputation || "Any"}</span>
                  </div>
                  <div className="flex items-center justify-between px-4 py-2.5">
                    <span className="text-xs text-gray-500">Max Revisions</span>
                    <span className="text-xs font-semibold text-gray-200">
                      {task.maxRevision !== undefined && task.maxRevision !== null ? Number(task.maxRevision) : "N/A"}
                    </span>
                  </div>
                  {hasAssignedMember && (
                    <div className="flex items-center justify-between px-4 py-2.5">
                      <span className="text-xs text-gray-500">Assigned To</span>
                      <span className="text-xs font-semibold font-mono text-gray-200">{shortAddr(task.member)}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Stake status */}
              {(task.isCreatorStakeLocked !== undefined || task.isMemberStakeLocked !== undefined) && (
                <div>
                  <p className="text-[11px] uppercase tracking-wider text-gray-600 font-semibold mb-2">Stake Status</p>
