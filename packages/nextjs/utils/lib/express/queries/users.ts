...
export async function getUserByAddress(walletAddress: string): Promise<User> {
  try {
    if (!walletAddress) {
    ...
    const normalizedInput = walletAddress.trim().replace(/\s/g, "").toLowerCase();

    const user = users.find(user => {
      const normalizedDb = user.walletAddress.trim().replace(/\s/g, "").toLowerCase();
    ...
    console.error(`error while fetching user by wallet address, error message: ${err}`);
    throw err;
  }
}

export async function getStartedFindUserByAddress(walletAddress: string) {
  try {
    if (!walletAddress) {
      throw new Error("wallet address not found");
    }
