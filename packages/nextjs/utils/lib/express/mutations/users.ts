...
export async function handleCreateAccount(
  ...
  try {
    ...
    const response = await axios.post<{ id: string }>(`${API_BASE}/api/users`, backendData, {
      headers: {
...
export async function handleUpdateAccount(
  ...
  try {
    const { data } = await axios.put(`${API_BASE}/api/users/${_id}`, backendData, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${jwtToken}`,
      },
...
export async function handleDelleteAccount(_id: string, jwtToken: string, walletAddress: string) {
  const user = await getUserById(_id);
  ...
  try {
    const response = await axios.delete(`${API_BASE}/api/users/${_id}`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${jwtToken}`,
      },
