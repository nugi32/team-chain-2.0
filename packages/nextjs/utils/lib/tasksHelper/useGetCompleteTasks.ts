...
export const useGetCompleteTasks = (address?: string) => {
  ...
  const [completeTasks, setCompleteTasks] = useState<CompleteTaskOutput[]>([]);
      ...
      try {
        ...
        const mergedTasks = await Promise.all(
          validTasks.map(async task => {
            try {
              console.log(`[useGetCompleteTasks] Processing Task #${task.taskId}:`, task);
              const dbTask = await getTaskBySmartContractId(task.taskId.toString());

              if (!dbTask) {
                console.warn(`[useGetCompleteTasks] No database task found for smart contract task #${task.taskId}`);

                console.log(`[useGetCompleteTasks] SC Task ID: #${String(task.taskId)}`);

                const dbTask = await getTaskBySmartContractId(task.taskId.toString());

                console.log(`[useGetCompleteTasks] DB RESULT for Task #${task.taskId}:`, dbTask);
                return null;
              }
              ...
              const completeTask: CompleteTaskOutput = {
                ...
                isMemberStakeLocked: task.isMemberStakeLocked,
                isCreatorStakeLocked: task.isCreatorStakeLocked,
              ...
              console.error(`[useGetCompleteTasks] Failed to load Task #${task.taskId}:`, error);
        ...
        const validTasksCheck = mergedTasks.filter((task): task is CompleteTaskOutput => task !== null);
  ...
  const getTasksByStatus = (status: TaskStatus) => {
    return completeTasks.filter(task => task.status === status);
  };

  const createdTasks = useMemo(() => getTasksByStatus(TaskStatus.Created), [completeTasks]);

  const activeTasks = useMemo(() => getTasksByStatus(TaskStatus.Active), [completeTasks]);

  const openRegistrationTasks = useMemo(() => getTasksByStatus(TaskStatus.OpenRegistration), [completeTasks]);

  const inProgressTasks = useMemo(() => getTasksByStatus(TaskStatus.InProgres), [completeTasks]);

  const completedTasks = useMemo(() => getTasksByStatus(TaskStatus.Completed), [completeTasks]);

  const cancelledTasks = useMemo(() => getTasksByStatus(TaskStatus.Cancelled), [completeTasks]);

  const creatorTasks = useMemo(() => {
    if (!creator) return [];

    return validTasks.filter(task => task.creator?.toLowerCase() === creator.toLowerCase());
  }, [validTasks, creator]);

  const memberTasks = useMemo(() => {
    if (!member) return [];

    return validTasks.filter(task => task.member?.toLowerCase() === member.toLowerCase());
  }, [validTasks, member]);

  const latestMemberTask = useMemo(() => {
    return memberTasks.length ? memberTasks[memberTasks.length - 1] : null;
  }, [memberTasks]);

  const latestCreatorTask = useMemo(() => {
    return creatorTasks.length ? creatorTasks[creatorTasks.length - 1] : null;
  }, [creatorTasks]);
  ...
  return {
    ...
    loading: {
      ...
      isLoading: loading.isLoading || isLoadingCompleteTasks,
    },
  };
};
