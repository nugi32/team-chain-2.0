...
export default function useDataContract() {
   ...
   */
  const getComponentWeightPercentages = useCallback(async (): Promise<ContractResponse<ComponentWeightPercentage>> => {
    try {
   ...
   */
  const getReputationPoints = useCallback(async (): Promise<ContractResponse<ReputationPoint>> => {
    try {
      const c = requireContract();
   ...
   */
  const getStateVariables = useCallback(async (): Promise<ContractResponse<StateVar>> => {
    try {
      const c = requireContract();
   ...
   */
  const getProjectCategories = useCallback(async (): Promise<ContractResponse<ProjectValueCategory>> => {
    try {
      const c = requireContract();
   ...
   */
  const getStakeUtils = useCallback(async (): Promise<ContractResponse<StakeUtil>> => {
    try {
      const c = requireContract();
  ...
  const getMemberStakeFromRewardPercentage = useCallback(async (): Promise<ContractResponse<bigint>> => {
    try {
      const c = requireContract();
  ...
  const getCreatorStakeFromProjectValuePercentage = useCallback(async (): Promise<ContractResponse<bigint>> => {
    try {
      const c = requireContract();
  ...
  const setProjectCategories = useCallback(
    async (
      ...
      try {
        const c = requireContract();
        const tx = await c.setProjectCategories(_low, _middleLow, _middle, _middleHigh, _high, _ultraHigh);
        const receipt = await tx.wait();
        return { success: true, txHash: tx.hash, receipt };
      } catch (error: any) {
        return { success: false, error: error.message };
  ...
  const setStakeUtils = useCallback(
    async (
      ...
      try {
        const c = requireContract();
        const tx = await c.setStakeUtils(_memberStakePercentageFromReward, _creatorStakePercentageFromProjectValue);
        const receipt = await tx.wait();
        return { success: true, txHash: tx.hash, receipt };
      } catch (error: any) {
        return { success: false, error: error.message };
