...
export default function useAddressRegistry() {
  ...
  const { data: deployedContractData } = useDeployedContractInfo("AddressRegistry");
  ...
  const updateCoreAddresses = useCallback(
    async (
      ...
      try {
        const c = requireContract();
        const tx = await c.updateCoreAddresses(_accessControlContract, _usersContract, _walletContract, _dataContract);
        const receipt = await tx.wait();
        return { success: true, txHash: tx.hash, receipt };
      } catch (error: any) {
        return { success: false, error: error.message };
   ...
   */
  const getAccessControlContract = useCallback(async (): Promise<ContractResponse<string>> => {
    try {
      const c = requireContract();
   ...
   */
  const getTaskComponents = useCallback(async (): Promise<ContractResponse<TaskComponents>> => {
    try {
      const c = requireContract();
