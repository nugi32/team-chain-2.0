...
import type { Applicant, ApplicantStatus, Submission, SubmissionStatus, TabKey } from "@/components/task-owner/types";
import { AnimatePresence, motion } from "framer-motion";
import { AlertCircle, ArrowLeft, Ban, Clock, FileText, Info, Lock, Users } from "lucide-react";
...
export default function TaskOwnerPage() {
  ...
  const confirm = (note?: string) => {
    ...
    if (type === "revision") {
      setSubmissions(prev =>
        prev.map(s => (s.id === id ? { ...s, status: "revision_requested", revisionNote: note } : s)),
      );
    }
    if (type === "dispute") {
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {[
            ...
            <div
              key={s.label}
              className={`flex items-center gap-2.5 rounded-xl border px-4 py-3 ${s.hi ? "border-indigo-500/30 bg-indigo-500/10" : "border-gray-800 bg-gray-900"}`}
            >
            ...
            <div className="space-y-5">
                ...
                <div className="flex items-center gap-1 bg-gray-800 rounded-xl p-1">
                  {(["all", "pending", "accepted", "rejected"] as const).map(f => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[11px] font-medium capitalize transition-colors ${appFilter === f ? "bg-gray-700 text-white" : "text-gray-500 hover:text-gray-300"}`}
                    >
                      {f}{" "}
              ...
              <div className="flex gap-2">
                {Array.from({ length: TASK.slots.total }).map((_, i) => (
                  <div
                    key={i}
                    className={`h-1.5 flex-1 rounded-full ${i < acceptedCount ? "bg-indigo-500" : "bg-gray-800"}`}
                  />
                ))}
              </div>
              <div className="space-y-3">
                ...
                {filteredApplicants.map(a => (
                  <ApplicantCard
                    ...
                    applicant={a}
                    slotsLeft={slotsLeft}
                    onAccept={() => setModal({ type: "accept", id: a.id })}
                    onReject={() => setModal({ type: "reject", id: a.id })}
                  />
                ))}
              </div>
            </div>
                ...
                <div className="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-4 flex gap-3">
                  ...
                  <p className="text-sm text-gray-400 leading-relaxed">
                    <span className="text-white font-semibold">
                      {pendingSubmissions} submission{pendingSubmissions > 1 ? "s" : ""}
                    </span>{" "}
                    awaiting your review. Approving releases the milestone reward. Requesting revision sends it back to
                    the worker. Disputing escalates to protocol arbitration.
                  </p>
                </div>
              )}
              {submissions.length === 0 && (
                <div className="text-center py-12 text-gray-600 text-sm">No submissions yet.</div>
              )}
              {submissions.map(s => (
                <SubmissionCard
                  key={s.id}
                  sub={s}
                  onApprove={() => setModal({ type: "approve", id: s.id })}
                  onRequestRevision={() => setModal({ type: "revision", id: s.id })}
                  onDispute={() => setModal({ type: "dispute", id: s.id })}
                />
              ))}
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-5 space-y-3">
                ...
                <p className="text-xs text-gray-500 leading-relaxed">
                  Closing the task will end all active commitments. Workers currently assigned will have their stakes
                  returned proportionally based on completed milestones. This action is recorded on-chain and cannot be
                  undone.
                </p>
                <button
                  onClick={() => setModal({ type: "close", id: "task" })}
