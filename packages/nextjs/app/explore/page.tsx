...
export default function Explore() {
    ...
    const maxStakeEth = useMemo(
        () => tasks.length === 0
            ? 200
            : Math.max(0.001, ...tasks.map(t => toEth(t.creatorStake))),
        [tasks],
    );
    ...
    const filtered = useMemo(() =>
        sortTasks(
            tasks.filter(t => {
                ...
                if (skillFilter.length > 0 && !skillFilter.some(s => t.skills?.includes(s)))
                    return false;
    ...
    const taskKey = (t: CompleteTaskOutput) =>
        t.expressId ?? String(t.smartContractId);

    // Top 2 by reward are "featured"
    const { featured, regular } = useMemo(() => {
        if (filtered.length === 0)
            return { featured: [] as CompleteTaskOutput[], regular: [] as CompleteTaskOutput[] };
