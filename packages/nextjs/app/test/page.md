...
<h1 className="text-2xl font-bold">Task Debug</h1>
      ...
      <div className="bg-blue-50 border border-blue-200 p-4 rounded">
        ...
        <div className="space-y-1 text-sm text-blue-800">
          <p>
            Requested Task ID: <span className="font-mono font-bold">{lastRequestedTaskId}</span>
          </p>
          <p>
            Currently Loading: <span className="font-bold">{isLoading ? "Yes ⏳" : "No ✓"}</span>
          </p>
          {isLoading && (
      ...
      <div>
        ...
        <pre className="bg-black text-green-400 p-4 rounded overflow-auto text-sm max-h-96">
          {JSON.stringify(
            ...
            (_, value) =>
              typeof value === "bigint" ? value.toString() : value === undefined ? "undefined (still loading?)" : value,
            2,
          )}
        </pre>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200 p-4 rounded text-sm text-amber-900">
        ...
        <ul className="list-disc list-inside space-y-1">
          <li>
            Data may be <code className="bg-amber-100 px-1 rounded">undefined</code> while loading from contract
          </li>
